...
test("AO provides the accepted catalog of at least 50 runnable presets", () => {
  assert.equal(AO_MACRO_CATALOG.length, 79);
...
test("catalog includes every property type and time-saving chained workflows", () => {
  ...
  for (const id of ["workspace-new", "workspace-new-preset", "todo-add", "todo-add-high", "todo-add-due", "todo-clear-completed", "table-project", "table-lab", "row-page", "page-column-first", "page-open", "vault-create", "vault-run"]) assert.ok(AO_MACRO_CATALOG.some((macro) => macro.id === id), id);
