...
test("account sync refreshes authenticated state for changes made on another device", () => {
  ...
  assert.match(source, /const refreshTimer = window\.setInterval\(refresh, 2_000\)/);
  assert.match(source, /if \(!serverChanged\) return/);
  assert.match(source, /JSON\.stringify\(collectUserStorage\(userId\)\) !== lastUploaded/);
