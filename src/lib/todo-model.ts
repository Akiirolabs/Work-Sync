...
export type TodoItem = { id: string; title: string; completed: boolean; priority: TodoPriority; dueDate?: string; createdAt: string };
export type AOTodoCommand = { action: string; title?: string; dueDate?: string; commands?: AOTodoCommand[] };

const makeId = () => crypto.randomUUID();
const today = () => new Date().toISOString().slice(0, 10);

export function createTodo(title: string, priority: TodoPriority = "normal", dueDate?: string): TodoItem | null {
  const cleanTitle = title.trim(); if (!cleanTitle) return null;
  return { id: makeId(), title: cleanTitle, completed: false, priority, dueDate: dueDate?.trim() || undefined, createdAt: new Date().toISOString() };
}
...
export function applyTodoCommand(items: TodoItem[], command: AOTodoCommand): TodoItem[] {
  ...
  if (["todo-add", "todo-add-high", "todo-add-today", "todo-add-due", "todo-follow-up", "todo-review"].includes(command.action)) {
    const item = createTodo(`${prefix}${command.title ?? ""}`, priority, dueDate); return item ? [...items, item] : items;
  }
  return items;
}
