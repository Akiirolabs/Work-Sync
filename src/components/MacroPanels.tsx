...
export function MacroPanels({ onAgent }: { onAgent: () => void }) {
  ...
  return <div ref={root} className={styles.root}>
    ...
    {radialOpen && <section ref={radial} className={`${styles.radial}${dragging ? ` ${styles.dragging}` : ""}`} style={radialPosition ? { left: radialPosition.left, top: radialPosition.top } as CSSProperties : undefined} role="dialog" aria-label="Circular Macro Panel"><button type="button" className={styles.radialCenter} aria-label="Move circular Macro Panel" onPointerDown={beginDrag}><span className={styles.liquidChrome} aria-hidden="true"><span className={styles.liquidVolume} /><span className={styles.liquidSheet} /><span className={styles.liquidReflection} /></span></button><MacroButtons items={items} radial onShowTooltip={showRadialTooltip} onHideTooltip={() => setRadialTooltip(null)} /></section>}
