...
type StepMode = "fixed" | "empty";
type RouteSection = "workspace" | "todo" | "tables" | "sources" | "verify" | "calendar";
...
// These actions stay out of the generic Presets catalog. `vault-create` remains
// available through its dedicated Create Text Preset controls in Macro and Vault.
const HIDDEN_CATALOG_ACTIONS = new Set(["workspace-add-code", "vault-create"]);
...
export function AOMacroMenu() {
  ...
  const [routeSection, setRouteSection] = useState<RouteSection>("workspace"); const [routePage, setRoutePage] = useState(""); const [routeAction, setRouteAction] = useState<"go" | "create" | "current" | "page">("go");
  ...
  const [builderStepMode, setBuilderStepMode] = useState<StepMode>("fixed");
  ...
  function readLocalData() {
    try { const parsed = JSON.parse(localStorage.getItem(userStorageKey(AO_MACROS_KEY)) ?? "[]") as AOMacroPreset[]; const cleaned = Array.isArray(parsed) ? parsed.filter((item) => !item.macroId || !HIDDEN_CATALOG_ACTIONS.has(item.macroId)).map((item) => item.main ? item : { ...item, icon: undefined }) : []; setPresets(cleaned); if (cleaned.length !== (Array.isArray(parsed) ? parsed.length : 0)) localStorage.setItem(userStorageKey(AO_MACROS_KEY), JSON.stringify(cleaned)); } catch { setPresets([]); }
  ...
  useEffect(() => {
    function openMenu() { readLocalData(); void refreshNotes(); setOpen(true); setView("macro"); setMacroMode("home"); }
  ...
  async function refreshNotes(): Promise<Note[]> { try { const list = await api<Note[]>("/api/v1/notes"); setNotes(list); return list; } catch { setNotes([]); return []; } }
  function showView(next: View) { readLocalData(); setView(next); setSelected(null); setCustomToRun(null); setVaultMenu(null); setMainTooltip(null); setError(""); setNotice(""); if (next === "macro") setMacroMode("home"); if (next === "route") { setRouteSection(permittedRoutes[sourceSection][0] ?? "workspace"); setRoutePage(""); setRouteAction("go"); } if (next === "vault") { setVaultRecent(false); setVaultCategory("All"); setVaultTextOnly(false); } if (next === "macro" || next === "vault") void refreshNotes(); }
  ...
  const sourceSection: RouteSection = pathname === "/todo" ? "todo" : pathname === "/tables" ? "tables" : pathname === "/history" ? "calendar" : pathname === "/sources" ? "sources" : pathname === "/verify" ? "verify" : "workspace";
  const permittedRoutes: Record<RouteSection, RouteSection[]> = { workspace: ["tables", "sources", "verify", "calendar"], todo: ["workspace", "tables", "calendar"], tables: ["workspace", "verify", "sources", "calendar"], sources: [], verify: [], calendar: [] };
  const routeLabels: Record<RouteSection, string> = { workspace: "Workspace", todo: "To Do", tables: "Tables", sources: "Sources", verify: "Verify", calendar: "Calendar Day Documents" };
  function routeOptions(section: RouteSection): Option[] { if (section === "workspace") return notes.map((note) => ({ value: note.id, label: note.title })); if (section === "todo") return todos.map((item) => ({ value: item.id, label: item.title })); if (section === "tables") return tables.map((item) => ({ value: item.id, label: item.name })); if (section === "calendar") return dayDocuments.map((item) => ({ value: item.id, label: `${item.name} · ${item.day}` })); if (section === "sources") return sourceResults.map((item) => ({ value: item.id, label: item.name })); return findings.map((item) => ({ value: item.id, label: item.name })); }
  async function runRoute() {
    const routes: Record<RouteSection, string> = { workspace: "/", todo: "/todo", tables: "/tables", sources: "/sources", verify: "/verify", calendar: "/history" };
    if (routeAction === "create") { if (routeSection === "workspace") return createWorkspaceNote("Untitled note"); if (routeSection === "todo") return sendTodoCommand({ action: "todo-add", title: "New To-Do" }); if (routeSection === "tables") return sendTableCommand({ action: "table-create", name: "New table" }); if (routeSection === "calendar") { const day = new Date().toISOString().slice(0, 10); if (!dayDocuments.some((item) => item.day === day)) saveDayDocument("", "__new_day__", day); return; } }
    if (routeAction === "page" && !routePage) { setError("Choose a destination item."); return; }
    if (routeSection === "workspace" && routePage) { const note = notes.find((item) => item.id === routePage); if (note) return openWorkspaceNote(note); }
    if (routeSection === "calendar" && routePage) { writeScoped(TIMELINE_OPEN_DOCUMENT_KEY, routePage); router.push("/history"); close(); return; }
    if (routeAction === "current") setNotice(`Using the current ${sourceSection === "calendar" ? "Day Document" : routeLabels[sourceSection]} as the source.`);
    router.push(routes[routeSection]); close();
  }
  async function createWorkspaceNote(title: string, content = "", projectId?: string) {
    const cleanTitle = title.trim() || "Untitled note"; const body = content.trim() ? `# ${cleanTitle}\n\n${content.trim()}` : `# ${cleanTitle}`;
    const note = await api<Note>("/api/v1/notes", { method: "POST", body: JSON.stringify({ title: cleanTitle, body }) });
    if (projectId) {
      ...
      writeScoped(NOTE_PROJECTS_KEY, { ...assignments, [note.id]: projectId }); window.dispatchEvent(new Event(AO_WORKSPACE_PROJECTS_EVENT));
    }
  ...
  function optionsFor(field: AOMacroField, context = values, macro = selected): Option[] {
    ...
    if (field.type === "subtask") return (todos.find((item) => item.id === context.taskId)?.subtasks ?? []).map((item) => ({ value: item.id, label: item.title }));
  ...
  function beginMacro(macro: AOMacroDefinition, overrides: Record<string, string> = {}) {
    ...
    if (macro.fields?.some((field) => field.type === "note")) void refreshNotes().then((freshNotes) => {
      if (!freshNotes.length) return;
      setValues((current) => {
        const next = { ...current };
        for (const field of macro.fields ?? []) if (field.type === "note" && !next[field.key]) next[field.key] = freshNotes[0]!.id;
        return next;
      });
    });
  }
  ...
  function saveText(text: string, label: string) { const clean = text.trim(); if (!clean) throw new Error("Choose content to save."); if (!presets.some((item) => isTextPreset(item) && item.text === clean)) savePresets([...presets, { id: crypto.randomUUID(), label: label.trim() || clean.slice(0, 52), text: clean, createdAt: new Date().toISOString() }]); setSelected(null); setNotice("Saved as text in Vault."); }
  function addBuilderStep() {
    const macro = AO_MACRO_CATALOG.find((item) => item.id === builderChoice); if (!macro || macro.category === "Vault") return;
    setBuilderSteps((current) => [...current, { id: crypto.randomUUID(), macroId: macro.id, mode: builderStepMode, values: builderStepMode === "fixed" ? initialValuesFor(macro) : {} }]);
  }
  function saveCustomMacro() {
    if (!builderName.trim() || !builderSteps.length) return;
    savePresets([...presets, { id: crypto.randomUUID(), label: builderName.trim(), text: "", steps: builderSteps.map(({ macroId, values: stepValues, mode }) => ({ macroId, values: stepValues, mode })), createdAt: new Date().toISOString() }]);
  ...
  async function runFlow(macro: AOMacroDefinition, context = values) {
    ...
    if (macro.action === "flow-note-project") {
      ...
      setNotice(`${selectedNote.title} added to the selected Project.`); setSelected(null); router.push(`/?project=${encodeURIComponent(projectId)}&note=${encodeURIComponent(selectedNote.id)}`); return;
    }
    if (macro.action === "flow-project-note") {
      ...
      const next = [...projects, project]; writeScoped(PROJECTS_KEY, next); setProjects(next); return createWorkspaceNote(get("title") || project.title, "", project.id);
    }
  ...
  async function runVault(macro: AOMacroDefinition) {
    ...
    if (macro.action === "vault-create") {
      const text = valueFor("text").trim();
      if (!text) throw new Error("Enter the text to save.");
      savePresets([...presets, { id: crypto.randomUUID(), label: valueFor("name").trim() || text.slice(0, 52), text, createdAt: now }]);
      setSelected(null); setNotice("Text preset saved to Vault."); return;
    }
    ...
    if (macro.action === "vault-run") { touchPreset(chosen.id); const destination = valueFor("destination"); if (destination === "new-note") await createWorkspaceNote(chosen.label, chosen.text); else if (destination === "current-note") { const note = notes.find((item) => item.id === valueFor("destinationNoteId")); if (!note) throw new Error("Choose a destination note."); const body = note.body.trim() ? `${note.body.trimEnd()}\n\n${chosen.text}` : chosen.text; await api<Note>(`/api/v1/notes/${note.id}`, { method: "PATCH", body: JSON.stringify({ body }) }); openWorkspaceNote({ ...note, body }); } else if (destination === "new-table") sendTableCommand({ action: "add-table", text: chosen.text }); else { if (!valueFor("page")) throw new Error("Choose a destination page."); sendTableCommand({ action: "page-append", page: valueFor("page"), text: chosen.text }); } return; }
  ...
  async function runSelected() {
    ...
    try {
      ...
      else if (selected.category === "To Do") { const task = todos.find((item) => item.id === valueFor("taskId")); if (selected.action === "todo-save-text") saveText(task?.title ?? "", task?.title ?? "To-Do"); else if (selected.action === "todo-subtask-save-text") { const subtask = task?.subtasks?.find((item) => item.id === valueFor("subtaskId")); saveText(subtask?.title ?? "", subtask?.title ?? "Subtask"); } else sendTodoCommand(todoCommandFor(selected, values)); }
  ...
  const filteredMacros = AO_MACRO_CATALOG.filter((item) => !HIDDEN_CATALOG_ACTIONS.has(item.action) && (!category || item.category === category) && (!query || `${item.label} ${item.description}`.toLowerCase().includes(query.toLowerCase())));
  const builderCatalog = AO_MACRO_CATALOG.filter((item) => item.category !== "Vault" && !HIDDEN_CATALOG_ACTIONS.has(item.action));
  ...
  const builderReady = Boolean(builderName.trim() && builderSteps.length && builderSteps.every((step) => step.mode === "empty" || (() => { const macro = AO_MACRO_CATALOG.find((item) => item.id === step.macroId); return (macro?.fields ?? []).every((field) => field.optional || Boolean(step.values[field.key]?.trim())); })()));
  ...
  return <div className={styles.root} ref={rootRef}>
    {open && <section className={`${styles.panel}${view === "macro" && macroMode === "builder" ? ` ${styles.builderPanel}` : ""}`} role="dialog" aria-label="AO macro key menu">
      ...
      {view === "macro" && !selected && macroMode === "builder" && <div className={styles.builder}>
        ...
        <div className={styles.builderAdd}><select aria-label="Preset element" value={builderChoice} onChange={(event) => setBuilderChoice(event.target.value)}>{AO_MACRO_CATEGORIES.filter((item) => item !== "Vault").map((item) => <optgroup key={item} label={item}>{builderCatalog.filter((macro) => macro.category === item).map((macro) => <option key={macro.id} value={macro.id}>{macro.label}</option>)}</optgroup>)}</select><select aria-label="Step type" value={builderStepMode} onChange={(event) => setBuilderStepMode(event.target.value as StepMode)}><option value="fixed">Fixed</option><option value="empty">Empty</option></select><button type="button" onClick={addBuilderStep}>Add step</button></div>
        <div className={styles.builderSteps}>{builderSteps.length ? builderSteps.map((step, index) => { const macro = AO_MACRO_CATALOG.find((item) => item.id === step.macroId)!; return <section key={step.id}><header><span>{index + 1}</span><strong>{macro.label} · {step.mode === "empty" ? "Empty" : "Fixed"}</strong><button type="button" aria-label={`Remove ${macro.label}`} onClick={() => setBuilderSteps((current) => current.filter((item) => item.id !== step.id))}><MacroIcon name="close" /></button></header>{step.mode === "empty" ? <p className={styles.hint}>Empty step — values are supplied when this macro runs.</p> : (macro.fields ?? []).map((field) => { const options = optionsFor(field, step.values, macro); const isSelect = options.length || ["table", "column", "column-type", "row", "page-column", "page", "preset", "destination", "note", "todo", "subtask", "heading", "project", "finding", "source-result", "day-document"].includes(field.type); const update = (value: string) => setBuilderSteps((current) => current.map((item) => item.id === step.id ? { ...item, values: { ...item.values, [field.key]: value, ...(field.type === "table" ? { columnId: "", rowId: "" } : {}) } } : item)); return <label key={`${step.id}-${field.key}`}>{field.label}{isSelect ? <select value={step.values[field.key] ?? ""} onChange={(event) => update(event.target.value)}><option value="">Choose…</option>{options.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select> : field.type === "textarea" ? <textarea value={step.values[field.key] ?? ""} onChange={(event) => update(event.target.value)} placeholder={field.placeholder} /> : <input type={field.type === "number" ? "number" : field.type === "date" ? "date" : "text"} value={step.values[field.key] ?? ""} onChange={(event) => update(event.target.value)} placeholder={field.placeholder} />}</label>; })}</section>; }) : <p>Add built-in preset elements to create a reusable sequence.</p>}</div>
      ...
      {view === "macro" && selected && <form className={styles.runner} onSubmit={(event) => { event.preventDefault(); void runSelected(); }}><p>{selected.description}</p>{(selected.fields ?? []).map((field) => { if (field.key === "dayDate" && valueFor("dayDocumentId") && valueFor("dayDocumentId") !== "__new_day__" && selected.action !== "flow-todo-day") return null; const options = optionsFor(field); const isSelect = options.length || ["table", "column", "column-type", "row", "page-column", "page", "preset", "destination", "note", "todo", "subtask", "heading", "project", "finding", "source-result", "day-document"].includes(field.type); return <label key={`${field.key}-${field.type}`}>{field.label}{isSelect ? <select aria-label={selected.action === "flow-verify-context" && field.type === "note" ? "Workspace note to verify" : field.label} value={values[field.key] ?? ""} onChange={(event) => setValues((current) => ({ ...current, [field.key]: event.target.value, ...(field.type === "table" ? { columnId: "", rowId: "" } : {}) }))}><option value="">{field.type === "note" && !options.length ? "No Workspace notes available" : "Choose…"}</option>{options.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select> : field.type === "textarea" ? <textarea value={values[field.key] ?? ""} onChange={(event) => setValues((current) => ({ ...current, [field.key]: event.target.value }))} placeholder={field.placeholder} /> : <input type={field.type === "number" ? "number" : field.type === "date" ? "date" : "text"} min={field.type === "number" ? 1 : undefined} max={field.type === "number" ? 100 : undefined} value={values[field.key] ?? ""} onChange={(event) => setValues((current) => ({ ...current, [field.key]: event.target.value }))} placeholder={field.placeholder} />}</label>; })}{valueFor("dayDocumentId") === "__new_day__" && dayDocuments.some((item) => item.day === valueFor("dayDate")) ? <p className={styles.error}>A Day Document already exists for that date. Choose an available date.</p> : null}{selected.action === "vault-run" && valueFor("destination") === "page" && <label>Page<select value={values.page ?? ""} onChange={(event) => setValues((current) => ({ ...current, page: event.target.value }))}><option value="">Choose…</option>{optionsFor({ key: "page", label: "Page", type: "page" }).map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label>}{error && <p className={styles.error}>{error}</p>}<button type="submit" disabled={busy || !requiredReady()}>{busy ? selected.action === "vault-create" ? "Saving…" : "Running…" : selected.action === "vault-create" ? "Save Text Preset" : "Run macro"}</button></form>}
      {view === "route" && <div className={styles.form}><p className={styles.hint}>Route {routeLabels[sourceSection]} content only to its permitted destinations.</p>{permittedRoutes[sourceSection].length ? <><label>Destination section<select aria-label="Route destination section" value={routeSection} onChange={(event) => { setRouteSection(event.target.value as RouteSection); setRoutePage(""); setRouteAction("go"); }}><option value="">Choose…</option>{permittedRoutes[sourceSection].map((section) => <option key={section} value={section}>{routeLabels[section]}</option>)}</select></label><label>Destination page / item<select aria-label="Route destination page" value={routePage} onChange={(event) => setRoutePage(event.target.value)}><option value="">Choose…</option>{routeOptions(routeSection).map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label><label>Action<select aria-label="Route action" value={routeAction} onChange={(event) => setRouteAction(event.target.value as typeof routeAction)}><option value="go">Go There</option><option value="create">Create New</option><option value="current">Add to Current</option><option value="page">Add to Page</option></select></label>{routeAction === "page" && <p className={styles.hint}>Choose the final destination page/item above before continuing.</p>}{error && <p className={styles.error}>{error}</p>}<button type="button" onClick={() => void runRoute()}>{routeAction === "go" ? "Go There" : routeAction === "create" ? "Create New" : routeAction === "current" ? "Add to Current" : "Add to Page"}</button></> : <p className={styles.hint}>This section has no permitted Route destinations.</p>}</div>}
