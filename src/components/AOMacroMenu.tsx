...
const REMOVED_PRESET_ACTIONS = new Set(["workspace-add-code", "vault-create"]);
...
export function AOMacroMenu() {
  ...
  async function runVault(macro: AOMacroDefinition) {
    ...
    if (macro.action === "vault-run") { touchPreset(chosen.id); const destination = valueFor("destination"); if (destination === "new-note") await createWorkspaceNote(chosen.label, chosen.text); else if (destination === "current-note") { const draft = currentDraft(); if (!draft.activeId) throw new Error("Choose a destination note."); const note = notes.find((item) => item.id === draft.activeId); if (!note) throw new Error("Choose a destination note."); await api<Note>(`/api/v1/notes/${note.id}`, { method: "PATCH", body: JSON.stringify({ body: note.body.trim() ? `${note.body.trimEnd()}\n\n${chosen.text}` : chosen.text }) }); openWorkspaceNote({ ...note, body: note.body.trim() ? `${note.body.trimEnd()}\n\n${chosen.text}` : chosen.text }); } else if (destination === "new-table") sendTableCommand({ action: "add-table", text: chosen.text }); else { if (!valueFor("page")) throw new Error("Choose a destination page."); sendTableCommand({ action: "page-append", page: valueFor("page"), text: chosen.text }); } return; }
  ...
  const filteredMacros = AO_MACRO_CATALOG.filter((item) => !REMOVED_PRESET_ACTIONS.has(item.action) && (!category || item.category === category) && (!query || `${item.label} ${item.description}`.toLowerCase().includes(query.toLowerCase())));
  const builderCatalog = AO_MACRO_CATALOG.filter((item) => item.category !== "Vault" && !REMOVED_PRESET_ACTIONS.has(item.action));
  ...
  return <div className={styles.root} ref={rootRef}>
    {open && <section className={`${styles.panel}${view === "macro" && macroMode === "builder" ? ` ${styles.builderPanel}` : ""}`} role="dialog" aria-label="AO macro key menu">
      ...
      {view === "vault" && !customToRun && <div className={styles.vault}>
        ...
        <div className={styles.vaultTextActions}><button type="button" className={vaultTextOnly ? styles.active : ""} onClick={() => { setVaultTextOnly(true); setVaultRecent(false); }}>View saved text</button></div>
