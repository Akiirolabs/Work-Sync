...
export function AgentSideChat({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const [sendDestination, setSendDestination] = useState<SendDestination | null>(null); const [workspaceTarget, setWorkspaceTarget] = useState<WorkspaceTarget | null>(null); const [workspaceNotes, setWorkspaceNotes] = useState<WorkspaceNote[]>([]); const [selectedNoteId, setSelectedNoteId] = useState("");
  ...
  function newChat() { setActiveChatId(crypto.randomUUID()); setMessages([]); setDraft(""); setError(""); setHistoryOpen(false); setSelectingOutput(false); setSelectedOutput(null); setSendDestination(null); setWorkspaceTarget(null); }
  function openChat(chat: SavedChat) { setActiveChatId(chat.id); setMessages(chat.messages); setHistoryOpen(false); setSelectingOutput(false); setSelectedOutput(null); setSendDestination(null); setWorkspaceTarget(null); }
  ...
  function dispatchDestination(destination: Destination) {
    ...
    if (destination === "tables" || destination === "page") { localStorage.setItem(userStorageKey(AO_TABLE_COMMAND_KEY), JSON.stringify({ action: "agent-output-page", title: shortTitle(text), text })); if (pathname === "/tables") window.dispatchEvent(new Event(AO_TABLE_COMMAND_EVENT)); else router.push("/tables"); }
    if (destination === "verify") { localStorage.setItem(userStorageKey(AGENT_VERIFY_CONTEXT_KEY), text); router.push("/verify"); }
    if (destination === "calendar") { localStorage.setItem(userStorageKey(AGENT_CALENDAR_CONTEXT_KEY), text); router.push("/history"); }
    setSelectedOutput(null); setSelectingOutput(false); setSendDestination(null); setWorkspaceTarget(null); onClose();
  }
  ...
  return <aside className={`${styles.panel} agent-side-chat`} aria-label="AO Agent side chat">
    <header><div><strong>AO Agent</strong><small>gpt-5-mini · text chat · no tools</small></div><nav aria-label="AO Agent actions"><button type="button" className={historyOpen ? styles.actionActive : ""} aria-label="Open AO Agent chat history" onClick={() => setHistoryOpen((value) => !value)}>History</button><button type="button" onClick={newChat}>New Chat</button><button type="button" className={selectingOutput ? styles.actionActive : ""} aria-label="Send an Agent output" title="Send an Agent output" onClick={() => { setHistoryOpen(false); setSelectedOutput(null); setSendDestination(null); setWorkspaceTarget(null); setSelectingOutput((value) => !value); }}><img src="/ao-agent.png" alt="" /></button><button type="button" aria-label="Close Agent" onClick={onClose}>×</button></nav></header>
    {historyOpen && <section className={styles.history} aria-label="AO Agent chat history"><header><strong>Chat history</strong></header><div>{history.length ? history.map((chat) => <button type="button" key={chat.id} className={chat.id === activeChatId ? styles.currentHistory : ""} onClick={() => openChat(chat)}><strong>{chat.name}</strong><small>{new Date(chat.updatedAt).toLocaleString()}</small></button>) : <p>No saved chats yet.</p>}</div></section>}
    {selectingOutput && !selectedOutput && <p className={styles.pickPrompt}>Select an Agent response to send.</p>}
    {selectedOutput && <section className={styles.sendMenu} aria-label="Send selected Agent output"><strong>Send this output to</strong>{sendDestination !== "workspace" ? <div className={styles.destinationChoices}><button type="button" onClick={() => dispatchDestination("todo")}>To-Do</button><button type="button" onClick={() => dispatchDestination("tables")}>Tables</button><button type="button" onClick={() => dispatchDestination("page")}>Page</button><button type="button" onClick={() => dispatchDestination("verify")}>Verify</button><button type="button" onClick={() => dispatchDestination("calendar")}>Calendar</button><button type="button" onClick={() => setSendDestination("workspace")}>Workspace Note</button></div> : <><button type="button" className={styles.backDestination} onClick={() => { setSendDestination(null); setWorkspaceTarget(null); }}>‹ Destinations</button><div className={styles.workspaceChoices}><button type="button" onClick={() => void sendToWorkspace("current")}>Current Note</button><button type="button" onClick={() => void sendToWorkspace("new")}>New Note</button><button type="button" onClick={() => void chooseWorkspaceNote()}>Choose Note</button></div>{workspaceTarget === "choose" && <label className={styles.noteChoice}>Workspace note<select value={selectedNoteId} onChange={(event) => setSelectedNoteId(event.target.value)}><option value="">Choose a note…</option>{workspaceNotes.map((note) => <option key={note.id} value={note.id}>{note.title}</option>)}</select><button type="button" disabled={!selectedNoteId} onClick={() => void sendToWorkspace("choose")}>Append to selected note</button></label>}</>}<button type="button" className={styles.cancelSend} onClick={() => { setSelectedOutput(null); setSendDestination(null); setWorkspaceTarget(null); }}>Cancel</button></section>}
