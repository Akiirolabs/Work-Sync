...
type TableIconName = "people" | "date" | "files" | "filter" | "group" | "appearance" | "settings" | "comments";
    ...
    <section className="ms-table-stage">
      ...
      <div className="ms-data-frame">
        <div className="ms-data-toolbar">
          ...
          <div className="ms-toolbar-end"><button title="Clipboard">▤</button><i /><button title="Appearance" aria-label="Appearance"><TableIcon name="appearance" /></button><button title="Settings" aria-label="Settings"><TableIcon name="settings" /></button><button title="Search" onClick={() => setFilterOpen(!filterOpen)}>⌕</button><button title="Comments" aria-label="Comments"><TableIcon name="comments" /></button></div>
        </div>
        ...
        <div className="ms-data-scroll">
          <table className="ms-data-grid">
            <thead><tr><th className="ms-row-number"><span className="ms-grid-select" /></th>{visibleColumns.map((column, columnIndex) => <th className={frozenIndex >= columnIndex ? "is-frozen" : ""} style={frozenIndex >= columnIndex ? { left: 42 + columnIndex * 145 } : undefined} key={column.id} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); const source = event.dataTransfer.getData("text/table-column"); if (source) changeTable((current) => moveColumn(current, source, column.id)); }}><button type="button" className="ms-column-drag" aria-label={`Move ${column.name} column`} draggable onDragStart={(event: DragEvent<HTMLButtonElement>) => event.dataTransfer.setData("text/table-column", column.id)}>⠿</button><span className="ms-column-type"><PropertyIcon type={column.type} /></span><input aria-label={`${column.name} column name`} value={column.name} onChange={(e) => changeTable((current) => ({ ...current, columns: current.columns.map((col) => col.id === column.id ? { ...col, name: e.target.value } : col) }))} /><button className="ms-column-options" aria-label={`Options for ${column.name} column`} onClick={(event) => { const rect = event.currentTarget.getBoundingClientRect(); const width = 303; const left = Math.max(8, Math.min(rect.right - width, window.innerWidth - width - 8)); const top = Math.max(8, Math.min(rect.bottom + 5, window.innerHeight - 520)); setPicker(null); setColumnMenu(columnMenu?.id === column.id ? null : { id: column.id, left, top }); setSummaryMenu(null); setMoreMenu(null); }}>⌄</button></th>)}<th className="ms-add-column"><button aria-label="Add column" onClick={() => { setColumnMenu(null); setPicker(picker === "add" ? null : "add"); }}>＋</button></th></tr></thead>
